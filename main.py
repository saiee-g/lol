...
@app.delete("/champions/delete/")
def delete_champ(name:str, db:Session = Depends(get_db)):
    existing_champ = crud.get_champ_name(db,name)
    if not existing_champ:
        raise HTTPException(status_code=404, detail="Champion not found")

    delete_success = crud.delete_champ(db,name)  

    if delete_success:
        return {"response": "champion deleted successfully"}
    else:
        raise HTTPException(status_code=500, detail="Error deleting champion")
